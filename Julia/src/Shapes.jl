...
ensure_ref{K,T}(b::Backend{K,T}, v::T) =
  NativeRef{K,T}(v)
ensure_ref{K,T}(b::Backend{K,T}, v::Vector{T}) =
  length(v) == 1 ?
    NativeRef{K,T}(v[1]) :
...
unite_ref{K,T}(b::Backend{K,T}, r0::UnionRef{K,T}, r1::EmptyRef{K,T}) = r0
unite_ref{K,T}(b::Backend{K,T}, r0::EmptyRef{K,T}, r1::UnionRef{K,T}) = r1
unite_ref{K,T}(b::Backend{K,T}, r0::GenericRef{K,T}, r1::EmptyRef{K,T}) = r0
unite_ref{K,T}(b::Backend{K,T}, r0::EmptyRef{K,T}, r1::GenericRef{K,T}) = r1

unite_refs{K,T}(b::Backend{K,T}, r::UnionRef{K,T}) =
  foldr((r0,r1)->unite_ref(b,r0,r1), EmptyRef{K,T}(), r.values)
unite_ref{K,T}(b::Backend{K,T}, r0::UnionRef{K,T}, r1::GenericRef{K,T}) =
...
intersect_ref{K,T}(b::Backend{K,T}, r0::EmptyRef{K,T}, r1::GenericRef{K,T}) = r0
intersect_ref{K,T}(b::Backend{K,T}, r0::GenericRef{K,T}, r1::UnionRef{K,T}) =
...
subtract_ref{K,T}(b::Backend{K,T}, r0::GenericRef{K,T}, r1::UniversalRef{K,T}) = EmptyRef{K,T}()
subtract_ref{K,T}(b::Backend{K,T}, r0::GenericRef{K,T}, r1::EmptyRef{K,T}) = r0
subtract_ref{K,T}(b::Backend{K,T}, r0::EmptyRef{K,T}, r1::GenericRef{K,T}) = r0
subtract_ref{K,T}(b::Backend{K,T}, r0::GenericRef{K,T}, r1::UnionRef{K,T}) =
...
collect_ref(ss::Shapes) = mapreduce(collect_ref, vcat, [], ss)
...
macro defproxy(name, parent, fields...)
  ...
  field_renames = map(esc ∘ Symbol ∘ ucfirst ∘ string, field_names)
...
length(path::PathOps) =
    let len = mapreduce(length, +, 0, path.ops)
        path.closed ?
...
realize_wall_no_openings(b::Backend, w::Wall) =
    let w_base_height = level_height(w.bottom_level),
        w_height = level_height(w.top_level) - w_base_height,
...
realize(b::Backend, s::Door) =
  let base_height = level_height(s.wall.bottom_level) + s.loc.y,
