...
global const world_cs = CS(eye(Mat4f))
...
LinAlg.cross(v::Vec, w::Vec) = _cross(v.raw, in_cs(w, v.cs).raw, v.cs)
_cross(a::Vec4f, b::Vec4f, cs::CS) =
  vxyz(a[2]*b[3]-a[3]*b[2], a[3]*b[1]-a[1]*b[3], a[1]*b[2]-a[2]*b[1], cs)

LinAlg.dot(v::Vec, w::Vec) = _dot(v.raw, in_cs(w, v.cs).raw)
