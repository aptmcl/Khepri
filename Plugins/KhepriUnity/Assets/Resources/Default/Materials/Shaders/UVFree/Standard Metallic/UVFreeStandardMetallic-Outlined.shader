...
Shader "UVFree/StandardMetallic/Single-Texture-Outline" {
	Properties {
	...
	SubShader {
		...
		Tags {
			...
			// Uncomment the following line if you are using the shader in
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			//
			...
			#ifdef _PARALLAXMAP
				half parallaxX = tex2D (_ParallaxMap, xUV).w;
				half parallaxY = tex2D (_ParallaxMap, yUV).w;
				half parallaxZ = tex2D (_ParallaxMap, zUV).w;
				...
				float2 parallaxOffset = ParallaxOffset (parallax, _Parallax, IN.viewDir);
			...
			fixed3 albedo = max(fixed3(0.001, 0.001, 0.001), tex.rgb * _Color.rgb);
			...
			fixed2 mg = fixed2(_Metallic, _Glossiness);
